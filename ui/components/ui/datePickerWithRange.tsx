...
export function DateTimePickerWithRange(props: DateTimePickerWithRangeProps) {
	...
	const [timeValue, setTimeValue] = React.useState<TimeRange>({
		from: dateTime?.from ? new Time(dateTime.from.getHours(), dateTime.from.getMinutes()) : new Time(0, 0),
		to: dateTime?.to ? new Time(dateTime.to.getHours(), dateTime.to.getMinutes()) : new Time(23, 59),
	});
...
export function DateTimePicker(props: DateTimePickerProps) {
	...
	const [timeValue, setTimeValue] = React.useState<TimeValue>(new Time(initialDate.getHours(), initialDate.getMinutes()));
	const [isOpen, setIsOpen] = React.useState<boolean>(false);
	...
	useEffect(() => {
		if (dateTime) {
			...
			setDate(newDate);
			setTimeValue(new Time(newDate.getHours(), newDate.getMinutes()));
		}
	}, [dateTime]);
